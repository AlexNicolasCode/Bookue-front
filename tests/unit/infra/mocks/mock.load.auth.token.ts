...
export const mockCookieManager = (value: string = faker.datatype.uuid()): jest.Mocked<typeof cookieManager> => {
    const mockedCookieManager = cookieManager as jest.Mocked<typeof cookieManager>
    jest.spyOn(mockedCookieManager, 'get').mockClear().mockReturnValueOnce(value as any)
