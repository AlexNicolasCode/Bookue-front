...
describe('Book details screen', () => {
  ...
  describe('when authenticated', () => {
    ...
    it('Should render correct book progress percentage', () => {
      ...
      mockLoadBookEndpoint(fakeBook)
      const fakeBookId = faker.datatype.uuid()
      const bookProcessPercentage = String((fakeBook.currentPage * 100) / fakeBook.pages).substring(
        0,
        4
      )
      cy.visit(`/book/${fakeBookId}/`)

      cy.getByTestId('book-details-process-percentage').should(
        'have.text',
        `${bookProcessPercentage}%`
      )
    })
